 ...
 */
flash_status_t flash_write(__IO uint32_t *flash_address, uint32_t *data,
  ...
  for (i = 0;
      (i < data_length) && (*flash_address <= (USER_FLASH_END_ADDRESS - 4));
      i++) {
    /* Device voltage range supposed to be [2.7V to 3.6V], the operation will
     be done by word */
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, *flash_address,
