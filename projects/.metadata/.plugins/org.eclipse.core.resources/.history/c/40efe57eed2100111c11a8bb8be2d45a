...
static JSONStatus_t populateCommonFields(const char *jobDoc,
    const size_t jobDocLength, int32_t fileIndex,
...
static JSONStatus_t populateMqttStreamingFields(const char *jobDoc,
    const size_t jobDocLength, AfrOtaJobDocumentFields_t *result);
...
static void buildIndexedFileQueryString(int32_t fileIndex,
    const char *queryString, size_t queryStringLength, char *result,
    size_t *resultLength);
 ...
 */
static bool uintFromString(const char *string, const uint32_t length,
    uint32_t *value);
...
bool populateJobDocFields(const char *jobDoc, const size_t jobDocLength,
    int32_t fileIndex, AfrOtaJobDocumentFields_t *result) {
  ...
  if (jsonResult == JSONSuccess) {
    jsonResult = JSON_SearchConst(jobDoc, jobDocLength, "afr_ota.protocols[0]",
        20U, &protocol, &protocolLength,
        NULL);
  }
  ...
  if ((jsonResult == JSONSuccess) && (protocolLength == 4U)) {
    if (strncmp("MQTT", protocol, protocolLength) == 0) {
      jsonResult = populateMqttStreamingFields(jobDoc, jobDocLength, result);
    } else {
      jsonResult = populateHttpStreamingFields(jobDoc, jobDocLength, fileIndex,
          result);
    }
  }
...
static JSONStatus_t populateCommonFields(const char *jobDoc,
  ...
  if (fileIndex <= 9) {
    buildIndexedFileQueryString(fileIndex, "filesize", 8U, queryString,
        &queryStringLength);
    jsonResult = searchUintValue(jobDoc, jobDocLength, queryString,
        queryStringLength, &(result->fileSize));
  } else {
    jsonResult = JSONIllegalDocument;
  }

  if (jsonResult == JSONSuccess) {
    buildIndexedFileQueryString(fileIndex, "fileid", 6U, queryString,
        &queryStringLength);
    jsonResult = searchUintValue(jobDoc, jobDocLength, queryString,
        queryStringLength, &(result->fileId));
  }

  if (jsonResult == JSONSuccess) {
    buildIndexedFileQueryString(fileIndex, "filepath", 8U, queryString,
        &queryStringLength);
    jsonResult = JSON_SearchConst(jobDoc, jobDocLength, queryString,
        queryStringLength, &jsonValue, &jsonValueLength,
        NULL);
  ...
  if (jsonResult == JSONSuccess) {
    buildIndexedFileQueryString(fileIndex, "certfile", 8U, queryString,
        &queryStringLength);
    jsonResult = JSON_SearchConst(jobDoc, jobDocLength, queryString,
        queryStringLength, &jsonValue, &jsonValueLength,
        NULL);
  ...
  if (jsonResult == JSONSuccess) {
    buildIndexedFileQueryString(fileIndex, "sig-sha256-ecdsa", 16U, queryString,
        &queryStringLength);
    jsonResult = JSON_SearchConst(jobDoc, jobDocLength, queryString,
        queryStringLength, &jsonValue, &jsonValueLength,
        NULL);
...
static JSONStatus_t populateMqttStreamingFields(const char *jobDoc,
  ...
  jsonResult = JSON_SearchConst(jobDoc, jobDocLength, "afr_ota.streamname", 18U,
...
static JSONStatus_t populateHttpStreamingFields(const char *jobDoc,
  ...
  buildIndexedFileQueryString(fileIndex, "fileType", 8U, queryString,
      &queryStringLength);
  jsonResult = searchUintValue(jobDoc, jobDocLength, queryString,
      queryStringLength, &(result->fileType));

  if (jsonResult == JSONSuccess) {
    buildIndexedFileQueryString(fileIndex, "auth_scheme", 11U, queryString,
        &queryStringLength);
    jsonResult = JSON_SearchConst(jobDoc, jobDocLength, queryString,
        queryStringLength, &jsonValue, &jsonValueLength,
        NULL);
  ...
  if (jsonResult == JSONSuccess) {
    buildIndexedFileQueryString(fileIndex, "update_data_url", 15U, queryString,
        &queryStringLength);
    jsonResult = JSON_SearchConst(jobDoc, jobDocLength, queryString,
        queryStringLength, &jsonValue, &jsonValueLength,
        NULL);
...
static JSONStatus_t searchUintValue(const char *jobDoc,
  ...
  jsonResult = JSON_SearchConst(jobDoc, jobDocLength, query, queryLength,
      &jsonValue, &jsonValueLength,
      NULL);

  if (jsonResult == JSONSuccess) {
    numConversionSuccess = uintFromString(jsonValue,
        (const uint32_t) jsonValueLength, value);
  }
