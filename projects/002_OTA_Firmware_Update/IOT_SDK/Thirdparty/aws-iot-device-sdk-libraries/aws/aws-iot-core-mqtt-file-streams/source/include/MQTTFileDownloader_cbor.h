...
#define OTA_CBOR_NUMBEROFBLOCKS_KEY       "n" /*!< Key for number of blocks. */
  ...
  bool CBOR_Decode_GetStreamResponseMessage(const uint8_t *messageBuffer,
      size_t messageSize, int32_t *fileId, int32_t *blockId, int32_t *blockSize,
      uint8_t *const*payload, size_t *payloadSize);
  ...
  bool CBOR_Encode_GetStreamRequestMessage(uint8_t *messageBuffer,
      size_t messageBufferSize, size_t *encodedMessageSize,
      const char *clientToken, uint32_t fileId, uint32_t blockSize,
      uint32_t blockOffset, const uint8_t *blockBitmap, size_t blockBitmapSize,
