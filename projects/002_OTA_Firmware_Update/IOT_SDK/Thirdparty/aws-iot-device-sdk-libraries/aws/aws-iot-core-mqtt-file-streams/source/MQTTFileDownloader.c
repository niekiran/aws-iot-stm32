 ...
 */
static size_t stringBuilder(char *buffer, size_t bufferSizeBytes,
 ...
 */
static uint16_t createTopic(char *topicBuffer, size_t topicBufferLen,
    const char *streamName, size_t streamNameLength, const char *thingName,
    size_t thingNameLength, const char *apiSuffix);
...
static MQTTFileDownloaderStatus_t handleCborMessage(const uint8_t *message,
    size_t messageLength, int32_t *fileId, int32_t *blockId, int32_t *blockSize,
    uint8_t *decodedData, size_t *decodedDataLength);
 ...
 */
static bool getNumberFromString(const char *str, size_t len, int32_t *num);
...
static uint16_t createTopic(char *topicBuffer, size_t topicBufferLen,
  ...
  topicLen = (uint16_t) stringBuilder(topicBuffer, topicBufferLen, topicParts);

  return topicLen;
}

MQTTFileDownloaderStatus_t mqttDownloader_init(
    MqttFileDownloaderContext_t *context, const char *streamName,
    size_t streamNameLength, const char *thingName, size_t thingNameLength,
    DataType_t dataType) {
  ...
  if ((streamName == NULL) || (streamNameLength == 0U) || (thingName == NULL)
      || (thingNameLength == 0U) || (context == NULL)) {
  ...
  if (initStatus == MQTTFileDownloaderSuccess) {
    ...
    (void) memset(context->topicStreamData, (int32_t) '\0',
    TOPIC_STREAM_DATA_BUFFER_SIZE);
    (void) memset(context->topicGetStream, (int32_t) '\0',
    ...
    context->topicStreamDataLength = createTopic(context->topicStreamData,
    TOPIC_STREAM_DATA_BUFFER_SIZE, streamName, streamNameLength, thingName,
        thingNameLength, streamDataApiSuffix);
  ...
  if (initStatus == MQTTFileDownloaderSuccess) {
    ...
    TOPIC_GET_STREAM_BUFFER_SIZE, streamName, streamNameLength, thingName,
        thingNameLength, getStreamApiSuffix);
...
size_t mqttDownloader_createGetDataBlockRequest(DataType_t dataType,
    uint16_t fileId, uint32_t blockSize, uint16_t blockOffset,
    uint32_t numberOfBlocksRequested, char *getStreamRequest,
  ...
  if ((getStreamRequestLength >= GET_STREAM_REQUEST_BUFFER_SIZE)
    ...
    (void) memset(getStreamRequest, (int32_t) '\0',
    ...
    if (dataType == DATA_TYPE_JSON) {
      ...
      GET_STREAM_REQUEST_BUFFER_SIZE, "{"
          ...
          "\"o\": %u,"
          "\"n\": %lu"
          "}", fileId, blockSize, blockOffset, numberOfBlocksRequested);
      ...
      GET_STREAM_REQUEST_BUFFER_SIZE, &requestLength, "rdy", fileId, blockSize,
          blockOffset,
          /* coverity[misra_c_2012_rule_7_4_violation] */
          (const uint8_t*) "MQ==", strlen("MQ=="), numberOfBlocksRequested);
    }
  }
...
static MQTTFileDownloaderStatus_t handleCborMessage(const uint8_t *message,
  ...
  cborDecodeRet = CBOR_Decode_GetStreamResponseMessage(message, messageLength,
      fileId, blockId, blockSize, &payload, &payloadSize);
...
static MQTTFileDownloaderStatus_t handleJsonMessage(uint8_t *message,
  ...
  result = JSON_Search((char* ) message, messageLength, fileIdQuery,
      fileIdQueryLength, &value, &fileBlockLength);
  ...
  if (handleStatus == MQTTFileDownloaderSuccess) {
    result = JSON_Search((char* ) message, messageLength, blockIdQuery,
        blockIdQueryLength, &value, &fileBlockLength);
  ...
  if (handleStatus == MQTTFileDownloaderSuccess) {
    result = JSON_Search((char* ) message, messageLength, blockSizeQuery,
        blockSizeQueryLength, &value, &fileBlockLength);
  ...
  if (handleStatus == MQTTFileDownloaderSuccess) {
    result = JSON_Search((char* ) message, messageLength, dataQuery,
        dataQueryLength, &dataValue, &dataValueLength);
  ...
  if (handleStatus == MQTTFileDownloaderSuccess) {
    ...
    mqttFileDownloader_CONFIG_BLOCK_SIZE, decodedDataLength,
        (const uint8_t*) dataValue, dataValueLength);
...
MQTTFileDownloaderStatus_t mqttDownloader_isDataBlockReceived(
    const MqttFileDownloaderContext_t *context, const char *topic,
    size_t topicLength) {
...
MQTTFileDownloaderStatus_t mqttDownloader_processReceivedDataBlock(
    const MqttFileDownloaderContext_t *context, uint8_t *message,
    size_t messageLength, int32_t *fileId, int32_t *blockId, int32_t *blockSize,
  ...
  if ((message != NULL) && (messageLength != 0U) && (data != NULL)
      && (dataLength != NULL) && (fileId != NULL) && (blockId != NULL)
      && (blockSize != NULL)) {
    (void) memset(data, (int32_t) '\0', mqttFileDownloader_CONFIG_BLOCK_SIZE);

    if (context->dataType == (uint8_t) DATA_TYPE_JSON) {
      decodingStatus = handleJsonMessage(message, messageLength, fileId,
          blockId, blockSize, data, dataLength);
    } else {
      decodingStatus = handleCborMessage(message, messageLength, fileId,
          blockId, blockSize, data, dataLength);
    }
  }
